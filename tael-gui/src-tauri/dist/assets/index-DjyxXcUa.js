(function(){const s=document.createElement("link").relList;if(s&&s.supports&&s.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))r(n);new MutationObserver(n=>{for(const l of n)if(l.type==="childList")for(const u of l.addedNodes)u.tagName==="LINK"&&u.rel==="modulepreload"&&r(u)}).observe(document,{childList:!0,subtree:!0});function a(n){const l={};return n.integrity&&(l.integrity=n.integrity),n.referrerPolicy&&(l.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?l.credentials="include":n.crossOrigin==="anonymous"?l.credentials="omit":l.credentials="same-origin",l}function r(n){if(n.ep)return;n.ep=!0;const l=a(n);fetch(n.href,l)}})();function pe(e,s=!1){return window.__TAURI_INTERNALS__.transformCallback(e,s)}async function $(e,s={},a){return window.__TAURI_INTERNALS__.invoke(e,s,a)}var z;(function(e){e.WINDOW_RESIZED="tauri://resize",e.WINDOW_MOVED="tauri://move",e.WINDOW_CLOSE_REQUESTED="tauri://close-requested",e.WINDOW_DESTROYED="tauri://destroyed",e.WINDOW_FOCUS="tauri://focus",e.WINDOW_BLUR="tauri://blur",e.WINDOW_SCALE_FACTOR_CHANGED="tauri://scale-change",e.WINDOW_THEME_CHANGED="tauri://theme-changed",e.WINDOW_CREATED="tauri://window-created",e.WINDOW_SUSPENDED="tauri://suspended",e.WINDOW_RESUMED="tauri://resumed",e.WEBVIEW_CREATED="tauri://webview-created",e.DRAG_ENTER="tauri://drag-enter",e.DRAG_OVER="tauri://drag-over",e.DRAG_DROP="tauri://drag-drop",e.DRAG_LEAVE="tauri://drag-leave"})(z||(z={}));async function fe(e,s){window.__TAURI_EVENT_PLUGIN_INTERNALS__.unregisterListener(e,s),await $("plugin:event|unlisten",{event:e,eventId:s})}async function X(e,s,a){var r;const n=(r=void 0)!==null&&r!==void 0?r:{kind:"Any"};return $("plugin:event|listen",{event:e,target:n,handler:pe(s)}).then(l=>async()=>fe(e,l))}const ve=200,L=500,Y='12px "BerkeleyMono", ui-monospace, Menlo, Consolas, monospace',me='11px "BerkeleyMono", ui-monospace, Menlo, Consolas, monospace',q="#141414",be="#181818",he="#2b2611",$e="#2a2a2a",J="#b5b5b1",K="#6f6f6c",Q="#ef4444",t={server:"http://127.0.0.1:7701",serviceFilter:"",statusFilter:"",lastWindow:"1h",textFilter:"",pinnedColumns:[],attrPickerOpen:!1,spanViewer:null,tab:"traces",prevTab:"traces",paused:!1,connection:"idle",error:null,streamId:crypto.randomUUID(),spans:[],selectedSpanIdx:null,services:[],selectedServiceIdx:null,liveTraceMap:new Map,liveTraces:[],selectedTraceIdx:null,timelineWindowMs:6e4,traceSpans:[],waterfallRows:[],selectedWaterfallIdx:null,currentTraceId:null,comments:[],commentDraft:"",evalRun:null,evalCases:[],selectedEvalIdx:null,evalFailuresOnly:!1,detailZoom:{start:0,end:1},liveZoom:{start:0,end:1}};let R=null,O=null,N=!1,F=null;const ee=document.querySelector("#app");if(!ee)throw new Error("missing #app");const p=ee;function c(){N||(N=!0,requestAnimationFrame(()=>{N=!1,Ae()}))}function i(e){return String(e??"").replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;")}function Se(e){const s=Date.parse(e);return Number.isFinite(s)?s:0}function P(e){return(Array.isArray(e)?e:Array.isArray(e?.spans)?e.spans:[]).map(a=>{const r=String(a.start_time??a.startTime??"-");return{traceId:String(a.trace_id??a.traceId??"-"),spanId:String(a.span_id??a.spanId??"-"),parentSpanId:a.parent_span_id??a.parentSpanId??null,service:String(a.service??"-"),operation:String(a.operation??"-"),durationMs:Number(a.duration_ms??a.durationMs??0),status:String(a.status??"-"),startTime:r,startTimeMs:Se(r),attributes:a.attributes&&typeof a.attributes=="object"?a.attributes:{},events:Array.isArray(a.events)?a.events:[]}})}function ye(e){return(Array.isArray(e?.services)?e.services:[]).map(s=>({name:String(s.name??"-"),spanCount:Number(s.span_count??s.spanCount??0),traceCount:Number(s.trace_count??s.traceCount??0),avgDurationMs:Number(s.avg_duration_ms??s.avgDurationMs??0),errorRate:Number(s.error_rate??s.errorRate??0)}))}function V(e){return(Array.isArray(e?.comments)?e.comments:[]).map(s=>({author:String(s.author??"-"),body:String(s.body??""),createdAt:String(s.created_at??s.createdAt??"-"),spanId:s.span_id??s.spanId??null}))}function Ie(e){return e?{runId:String(e.run_id??e.runId??"-"),suiteId:String(e.suite_id??e.suiteId??"-"),status:String(e.status??"-"),caseCount:e.case_count??e.caseCount??null,observedCases:Number(e.observed_cases??e.observedCases??0),scoredCases:Number(e.scored_cases??e.scoredCases??0),passedCases:Number(e.passed_cases??e.passedCases??0),failedCases:Number(e.failed_cases??e.failedCases??0),costUsd:Number(e.cost_usd??e.costUsd??0),avgScores:e.avg_scores??e.avgScores??{}}:null}function we(e){return(Array.isArray(e?.cases)?e.cases:[]).map(s=>({caseId:String(s.case_id??s.caseId??"-"),status:String(s.status??"-"),traceId:s.trace_id??s.traceId??null,durationMs:s.duration_ms??s.durationMs??null,costUsd:Number(s.cost_usd??s.costUsd??0),scores:s.scores??{},comments:V({comments:s.comments})}))}function w(e){const s=["#facc15","#62a9ff","#52d284","#b78cff","#f59e8c","#5ad1c9","#e0a3ff","#8fc4ff","#d4b483","#ff9ab0"];let a=0;for(const r of e)a=a*31+r.charCodeAt(0)>>>0;return s[a%s.length]}function A(e){return e>=500?"danger":e>=100?"warn":"ok"}function C(e){return e==="error"||e==="fail"?"danger":e==="ok"||e==="pass"?"ok":"muted"}function te(e){return(e.includes("T")?e.split("T")[1]:e).replace(/Z$/,"").slice(0,12)}function k(e,s=16){return e.length>s?`${e.slice(0,s)}...`:e}function ge(e,s){const a=e.attributes[s];return a==null?"":typeof a=="string"?a:JSON.stringify(a)}function Te(){const e=M()??g(),s=new Set,a=[],r=n=>{if(n)for(const l of Object.keys(n.attributes))s.has(l)||(s.add(l),a.push(l))};r(e);for(const n of t.spans)r(n);for(const n of t.traceSpans)r(n);return a}function Ee(e){const s=t.pinnedColumns.indexOf(e);s>=0?t.pinnedColumns.splice(s,1):t.pinnedColumns.push(e)}function U(){const e=t.textFilter.trim().toLowerCase();return e?t.spans.filter(s=>s.service.toLowerCase().includes(e)||s.operation.toLowerCase().includes(e)||s.traceId.toLowerCase().includes(e)||s.status.toLowerCase().includes(e)):t.spans}function j(){const e=t.textFilter.trim().toLowerCase();return e?t.liveTraces.filter(s=>s.service.toLowerCase().includes(e)||s.operation.toLowerCase().includes(e)||s.traceId.toLowerCase().includes(e)||(s.hasError?"error":"ok").includes(e)):t.liveTraces}function D(){const e=t.textFilter.trim().toLowerCase();return t.evalCases.filter(s=>t.evalFailuresOnly&&s.status!=="fail"?!1:e?s.caseId.toLowerCase().includes(e)||s.status.toLowerCase().includes(e)||(s.traceId??"").toLowerCase().includes(e):!0)}function Ce(e){if(e.length===0)return[];const s=Math.min(...e.map(d=>d.startTimeMs)),a=Math.max(...e.map(d=>d.startTimeMs+d.durationMs)),r=Math.max(a-s,1),n=new Map,l="__root__";e.forEach((d,m)=>{const b=d.parentSpanId??l,y=n.get(b)??[];y.push(m),n.set(b,y)});const u=[],f=[{parent:l,depth:0}];for(;f.length>0;){const d=f.pop(),m=n.get(d.parent)??[];for(const b of[...m].reverse()){const y=e[b];u.push({spanIdx:b,depth:d.depth,offsetPct:I((y.startTimeMs-s)/r,0,1),widthPct:I(y.durationMs/r,.005,1)}),f.push({parent:y.spanId,depth:d.depth+1})}}const v=new Set(u.map(d=>d.spanIdx));return e.forEach((d,m)=>{v.has(m)||u.push({spanIdx:m,depth:0,offsetPct:I((d.startTimeMs-s)/r,0,1),widthPct:I(d.durationMs/r,.005,1)})}),u}function I(e,s,a){return Math.max(s,Math.min(a,e))}function se(e){for(const s of e){const a=s.startTimeMs+s.durationMs,r=t.liveTraceMap.get(s.traceId);if(!r){t.liveTraceMap.set(s.traceId,{traceId:s.traceId,service:s.service,operation:s.operation,startTimeMs:s.startTimeMs,endTimeMs:a,durationMs:s.durationMs,spanCount:1,hasError:s.status==="error"});continue}r.startTimeMs=Math.min(r.startTimeMs,s.startTimeMs),r.endTimeMs=Math.max(r.endTimeMs,a),r.durationMs=r.endTimeMs-r.startTimeMs,r.spanCount+=1,r.hasError||=s.status==="error",s.parentSpanId||(r.service=s.service,r.operation=s.operation)}if(t.liveTraces=[...t.liveTraceMap.values()].sort((s,a)=>s.startTimeMs-a.startTimeMs),t.liveTraces.length>L){const s=t.liveTraces.slice(0,t.liveTraces.length-L);for(const a of s)t.liveTraceMap.delete(a.traceId);t.liveTraces=t.liveTraces.slice(-L)}}async function W(){const e=await $("query_traces",{server:t.server,request:{service:t.serviceFilter||null,status:t.statusFilter||null,last:t.lastWindow||"1h",limit:200,text:t.textFilter||null}});t.spans=P(e),se(t.spans)}async function Z(){t.services=ye(await $("list_services",{server:t.server}))}async function B(){const e=await $("eval_runs",{server:t.server}),s=Array.isArray(e?.runs)?e.runs[0]:null,a=s?.run_id??s?.runId;if(!a){t.evalRun=null,t.evalCases=[];return}const r=await $("eval_status",{server:t.server,runId:a});t.evalRun=Ie(r?.run??r),t.evalCases=we(await $("eval_cases",{server:t.server,runId:a}))}async function E(e){t.prevTab=t.tab==="detail"?t.prevTab:t.tab,t.tab="detail",t.currentTraceId=e,t.selectedWaterfallIdx=null,t.traceSpans=[],t.waterfallRows=[],t.comments=[],t.detailZoom={start:0,end:1},t.error=null,c();try{const[s,a]=await Promise.all([$("get_trace",{server:t.server,traceId:e}),$("get_comments",{server:t.server,traceId:e})]);t.traceSpans=P(s),t.waterfallRows=Ce(t.traceSpans),t.selectedWaterfallIdx=t.waterfallRows.length>0?0:null,t.comments=V(a)}catch(s){t.error=String(s)}c()}async function Me(){if(!t.currentTraceId||!t.commentDraft.trim())return;const e=g();try{await $("add_comment",{server:t.server,request:{traceId:t.currentTraceId,body:t.commentDraft.trim(),author:"gui",spanId:e?.spanId??null}}),t.commentDraft="",t.comments=V(await $("get_comments",{server:t.server,traceId:t.currentTraceId}))}catch(s){t.error=String(s)}c()}async function x(){t.error=null,t.connection="checking",t.streamId=crypto.randomUUID(),c();try{await $("healthz",{server:t.server}),t.connection="loading",await Promise.all([W(),Z(),B()]),await _e(),t.connection="connected"}catch(e){t.connection="error",t.error=String(e)}c()}async function _e(){await $("start_live_stream",{server:t.server,service:t.serviceFilter||null,status:t.statusFilter||null,streamId:t.streamId})}async function xe(){R?.(),O?.(),R=await X("tael://live-spans",e=>{if(!(e.payload.streamId!==t.streamId||t.paused))try{const s=P(JSON.parse(e.payload.data));if(s.length===0)return;se(s),t.spans=[...s,...t.spans].slice(0,ve),t.error=null,c()}catch{}}),O=await X("tael://live-status",e=>{e.payload.streamId===t.streamId&&(t.connection=e.payload.status,e.payload.message&&(t.error=e.payload.message),c())})}function H(){const e=j();return t.selectedTraceIdx==null?null:e[t.selectedTraceIdx]??null}function M(){const e=U();return t.selectedSpanIdx==null?null:e[t.selectedSpanIdx]??null}function g(){if(t.selectedWaterfallIdx==null)return null;const e=t.waterfallRows[t.selectedWaterfallIdx];return e?t.traceSpans[e.spanIdx]:null}function _(e,s){return`<button class="tab ${t.tab===e?"active":""}" data-tab="${e}">${s}</button>`}function Ae(){p.innerHTML=`
  ...
  `,Pe(),Ve()}function ke(){return t.tab==="services"?Ne():t.tab==="evals"?Re():t.tab==="timeline"?Fe():t.tab==="detail"?De():Le()}function Le(){const e=U(),s=M(),a=t.pinnedColumns.map(r=>`<th>${i(r)}</th>`).join("");return`
    <section class="split vertical">
            ...
            <tbody>
              ${e.map((r,n)=>`
                <tr class="${t.selectedSpanIdx===n?"selected":""}" data-span-idx="${n}">
                  ...
                  ${t.pinnedColumns.map(l=>{const u=ge(r,l);return`<td class="${u?"attr-cell":"muted"}">${i(u||"-")}</td>`}).join("")}
                </tr>
              `).join("")}
            </tbody>
          </table>
      ...
      <aside class="pane detail-pane">${s?ae(s):'<div class="empty">No span selected.</div>'}</aside>
    </section>
  ...
  `}function Oe(e){return`
    ...
    ${e.comments.length?`<div class="comment-list">${e.comments.map(re).join("")}</div>`:""}
  ...
  `}function De(){const e=g();return`
    <section class="detail-grid">
      ...
      <aside class="pane span-side">
        ${e?ae(e):'<div class="empty">No span selected.</div>'}
      </aside>
      <section class="pane comments-pane">
        <div class="pane-title"><span>Comments</span><span>${t.comments.length}</span></div>
        <div class="comment-list">${t.comments.map(re).join("")||'<div class="empty compact">No comments.</div>'}</div>
  ...
  `}function re(e){const s=te(e.createdAt).slice(0,8);return`
    <div class="comment">
  ...
  `}function Pe(){p.querySelector("#server-input")?.addEventListener("change",e=>{t.server=e.currentTarget.value.trim()}),p.querySelector("#service-input")?.addEventListener("change",e=>{t.serviceFilter=e.currentTarget.value.trim(),x()}),p.querySelector("#status-input")?.addEventListener("change",e=>{t.statusFilter=e.currentTarget.value,x()}),p.querySelector("#last-input")?.addEventListener("change",e=>{t.lastWindow=e.currentTarget.value.trim()||"1h",W().catch(s=>t.error=String(s)).finally(c)}),p.querySelector("#filter-input")?.addEventListener("input",e=>{t.textFilter=e.currentTarget.value,t.selectedSpanIdx=null,t.selectedTraceIdx=null,t.selectedEvalIdx=null,c()}),p.querySelector("#clear-filter-btn")?.addEventListener("click",()=>{t.textFilter="",c()}),p.querySelector("#connect-btn")?.addEventListener("click",x),p.querySelector("#refresh-btn")?.addEventListener("click",()=>{Promise.all([W(),Z(),B()]).catch(e=>t.error=String(e)).finally(c)}),p.querySelector("#pause-btn")?.addEventListener("click",()=>{t.paused=!t.paused,c()}),p.querySelectorAll("[data-tab]").forEach(e=>{e.addEventListener("click",()=>{t.tab=e.dataset.tab,c()})}),p.querySelectorAll("[data-span-idx]").forEach(e=>{e.addEventListener("click",()=>{t.selectedSpanIdx=Number(e.dataset.spanIdx),c()}),e.addEventListener("dblclick",()=>{const s=U()[Number(e.dataset.spanIdx)];s&&E(s.traceId)})}),p.querySelector("#open-selected-trace-btn")?.addEventListener("click",()=>{const e=M()??g();e&&E(e.traceId)}),p.querySelector("#pin-columns-btn")?.addEventListener("click",()=>{t.attrPickerOpen=!0,c()}),p.querySelector("#view-span-btn")?.addEventListener("click",()=>{const e=M()??g();e&&(t.spanViewer=e,c())}),p.querySelector("#close-attr-picker-btn")?.addEventListener("click",()=>{t.attrPickerOpen=!1,c()}),p.querySelectorAll("[data-attr-key]").forEach(e=>{e.addEventListener("change",()=>{const s=e.dataset.attrKey;s&&Ee(s),c()})}),p.querySelector("#close-span-viewer-btn")?.addEventListener("click",()=>{t.spanViewer=null,c()}),p.querySelectorAll("[data-service-idx]").forEach(e=>{e.addEventListener("click",()=>{const s=t.services[Number(e.dataset.serviceIdx)];s&&(t.selectedServiceIdx=Number(e.dataset.serviceIdx),t.serviceFilter=s.name,t.tab="traces",x())})}),p.querySelector("#failures-only-btn")?.addEventListener("click",()=>{t.evalFailuresOnly=!t.evalFailuresOnly,t.selectedEvalIdx=null,c()}),p.querySelectorAll("[data-eval-idx]").forEach(e=>{e.addEventListener("click",()=>{t.selectedEvalIdx=Number(e.dataset.evalIdx),c()}),e.addEventListener("dblclick",()=>{const s=D()[Number(e.dataset.evalIdx)];s?.traceId&&E(s.traceId)})}),p.querySelector("#open-eval-trace-btn")?.addEventListener("click",()=>{const e=t.selectedEvalIdx==null?null:D()[t.selectedEvalIdx];e?.traceId&&E(e.traceId)}),p.querySelector("#open-selected-live-trace-btn")?.addEventListener("click",()=>{const e=H();e&&E(e.traceId)}),p.querySelector("#back-btn")?.addEventListener("click",()=>{t.tab=t.prevTab,c()}),p.querySelector("#comment-input")?.addEventListener("input",e=>{t.commentDraft=e.currentTarget.value}),p.querySelector("#submit-comment-btn")?.addEventListener("click",Me)}function Ve(){const e=p.querySelector("#timeline-canvas");e&&Ue(e);const s=p.querySelector("#waterfall-canvas");s&&je(s)}function ne(e){const s=e.getBoundingClientRect(),a=window.devicePixelRatio||1;e.width=Math.max(1,Math.floor(s.width*a)),e.height=Math.max(1,Math.floor(s.height*a));const r=e.getContext("2d");if(!r)throw new Error("2d canvas unavailable");return r.scale(a,a),r.clearRect(0,0,s.width,s.height),r}function Ue(e){const s=j(),a=ne(e),r=e.getBoundingClientRect(),n=260,l=26,u=34,f=Math.max(r.width-n-96,1),d=s.reduce((o,S)=>Math.max(o,S.endTimeMs),0)-t.timelineWindowMs,m=d+t.timelineWindowMs*t.liveZoom.start,b=d+t.timelineWindowMs*t.liveZoom.end,y=Math.max(b-m,1);a.fillStyle=q,a.fillRect(0,0,r.width,r.height),ie(a,n,12,f,m,b);const T=s.filter(o=>o.endTimeMs>=m&&o.startTimeMs<=b);T.forEach((o,S)=>{const h=u+S*l;if(h>r.height-l)return;const de=s.indexOf(o)===t.selectedTraceIdx;le(a,0,h-3,r.width,l,de),a.fillStyle=w(o.service),a.font=Y,a.fillText(`${o.service} ${o.operation}`.slice(0,34),18,h+13);const G=n+I((o.startTimeMs-m)/y,0,1)*f,ue=Math.max(2,o.durationMs/y*f);a.fillStyle=o.hasError?Q:w(o.service),oe(a,G,h,Math.min(ue,n+f-G),14,3),a.fill(),a.fillStyle=J,a.fillText(`${o.durationMs.toFixed(0)}ms`,n+f+14,h+12),a.fillStyle=K,a.fillText(String(o.spanCount),n+f+68,h+12)}),e.onmousemove=o=>{const S=Math.floor((o.offsetY-u)/l),h=T[S];e.title=h?`${h.service} ${h.operation} ${h.durationMs.toFixed(1)}ms`:""},e.onclick=o=>{const S=Math.floor((o.offsetY-u)/l),h=T[S];h&&(t.selectedTraceIdx=s.indexOf(h),c())},e.ondblclick=()=>{const o=H();o&&E(o.traceId)},e.onwheel=o=>{o.preventDefault();const S=o.deltaY>0?1.18:.84;ce(t.liveZoom,S,o.offsetX/r.width),c()}}function je(e){const s=ne(e),a=e.getBoundingClientRect(),r=t.waterfallRows,n=300,l=28,u=36,f=Math.max(a.width-n-92,1);s.fillStyle=q,s.fillRect(0,0,a.width,a.height),ie(s,n,12,f,t.detailZoom.start,t.detailZoom.end,!0),r.forEach((v,d)=>{const m=t.traceSpans[v.spanIdx],b=u+d*l;if(b>a.height-l)return;const y=t.selectedWaterfallIdx===d;le(s,0,b-4,a.width,l,y),s.font=Y,s.fillStyle=w(m.service),s.fillText(`${" ".repeat(v.depth*2)}${m.service} ${m.operation}`.slice(0,42),18,b+13);const T=t.detailZoom.end-t.detailZoom.start,o=n+(v.offsetPct-t.detailZoom.start)/T*f,S=Math.max(2,v.widthPct/T*f);o+S<n||o>n+f||(s.fillStyle=m.status==="error"?Q:w(m.service),oe(s,I(o,n,n+f),b,Math.min(S,n+f-o),15,3),s.fill(),s.fillStyle=J,s.fillText(`${m.durationMs.toFixed(0)}ms`,n+f+14,b+12))}),e.onclick=v=>{const d=Math.floor((v.offsetY-u)/l);r[d]&&(t.selectedWaterfallIdx=d,c())},e.ondblclick=()=>{const v=g();v&&(t.selectedSpanIdx=t.spans.findIndex(d=>d.spanId===v.spanId))},e.onwheel=v=>{v.preventDefault(),ce(t.detailZoom,v.deltaY>0?1.18:.84,v.offsetX/a.width),c()}}function ie(e,s,a,r,n,l,u=!1){e.strokeStyle=$e,e.fillStyle=K,e.font=me,e.beginPath(),e.moveTo(s,a+12),e.lineTo(s+r,a+12),e.stroke();for(let f=0;f<=4;f+=1){const v=s+r*f/4;e.beginPath(),e.moveTo(v,a+7),e.lineTo(v,a+17),e.stroke();const d=n+(l-n)*f/4,m=u?`${Math.round(d*100)}%`:f===4?"now":`-${Math.round((l-d)/1e3)}s`;e.fillText(m,v+4,a+7)}}function le(e,s,a,r,n,l){e.fillStyle=l?he:a%56===0?be:q,e.fillRect(s,a,r,n)}function oe(e,s,a,r,n,l){const u=Math.min(l,r/2,n/2);e.beginPath(),e.moveTo(s+u,a),e.arcTo(s+r,a,s+r,a+n,u),e.arcTo(s+r,a+n,s,a+n,u),e.arcTo(s,a+n,s,a,u),e.arcTo(s,a,s+r,a,u),e.closePath()}function ce(e,s,a){const r=e.end-e.start,n=I(r*s,.03,1),l=e.start+r*I(a,0,1);e.start=I(l-n*a,0,1-n),e.end=e.start+n}window.addEventListener("keydown",e=>{if(!(e.target instanceof HTMLInputElement||e.target instanceof HTMLSelectElement)){if(e.key==="Escape"&&t.spanViewer){t.spanViewer=null,c();return}if(e.key==="Escape"&&t.attrPickerOpen){t.attrPickerOpen=!1,c();return}if(e.key==="1"&&(t.tab="traces"),e.key==="2"&&(t.tab="services"),e.key==="3"&&(t.tab="evals"),e.key==="4"&&(t.tab="timeline"),e.key==="Escape"&&t.tab==="detail"&&(t.tab=t.prevTab),e.key===" "&&(t.paused=!t.paused),e.key==="a"&&(M()||g())&&(t.attrPickerOpen=!0),e.key==="v"){const s=M()??g();s&&(t.spanViewer=s)}c()}});window.addEventListener("resize",c);async function Ze(){try{const e=await $("initial_server");e.trim()&&(t.server=e.trim())}catch(e){console.warn("failed to load initial server",e)}await xe(),x()}Ze();F=window.setInterval(()=>{t.connection==="connected"&&Promise.all([Z(),B()]).catch(e=>{t.error=String(e),c()})},5e3);window.addEventListener("beforeunload",()=>{R?.(),O?.(),F!=null&&window.clearInterval(F)});
